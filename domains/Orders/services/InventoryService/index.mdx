...
Whenever a new batch of product stock is registered and becomes available for fulfillment, the service emits an `InventoryCreated` event so downstream services (like Orders) can re-evaluate backordered items.
